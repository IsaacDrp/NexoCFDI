...
public interface IngestedEmailRepository {
    ...
    Optional<IngestedEmail> findById(UUID id);
    IngestedEmail update(IngestedEmail email);
    List<IngestedEmail> findByUserAndReceivedBetween(UUID userId, OffsetDateTime from, OffsetDateTime to);
    List<String> findStorageKeysByUserAndPeriod(UUID userId, OffsetDateTime from, OffsetDateTime to);
