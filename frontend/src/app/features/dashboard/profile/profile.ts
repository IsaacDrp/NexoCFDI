...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.userService.getProfile().subscribe({
      next: (user) => {
        this.ngZone.run(() => {
          this.form.patchValue({ ...user, personType: user.personType ?? null, regimenFiscal: user.regimenFiscal ?? null });
          if (user.personType) {
      ...
      error: (err: HttpErrorResponse) => {
        this.ngZone.run(() => {
          if (err.status !== 404) this.toast.error('Error al cargar el perfil');
          this.loading.set(false);
        });
      },
    });
  ...
  save() {
    ...
    const request: RegisterUserRequest = {
      firstName: v.firstName!, paternalSurname: v.paternalSurname!,
      maternalSurname: v.maternalSurname ?? '', rfc: v.rfc!,
      razonSocial: v.razonSocial!, postalCode: v.postalCode!,
      personType: v.personType!, regimenFiscal: v.regimenFiscal!,
    };
    ...
    this.userService.saveProfile(request).subscribe({
      ...
      error: () => {
        this.ngZone.run(() => { this.saving.set(false); this.toast.error('Error al guardar el perfil'); });
      },
    });
  }
}
